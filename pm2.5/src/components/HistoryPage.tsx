...
export default function HistoryPage() {
  ...
  const exportCSV = () => {
    ...
    const rows = points
      .map((p) => {
        const pT = p.timestamp.getTime();
        while (
          vIdx < vehicleHistory.length - 1 &&
          vehicleHistory[vIdx + 1].timestamp.getTime() <= pT
        ) {
          vIdx++;
        }
        const matched =
          vehicleHistory.length > 0 &&
          vehicleHistory[vIdx].timestamp.getTime() <= pT
            ? vehicleHistory[vIdx]
            : null;

        // Note: hotspot tier is a whole-session/whole-window classification
        // (see classifyHotspotSession), not computed per-reading, so this
        // value is intentionally identical on every exported row.
        const tier = getHotspotTierLabel(hotspotSession.hotspotTier);

        if (!hasVehicles) {
          return `${p.timestamp.toISOString()},${p.pm25},${tier}`;
        }

        const vRow = matched
          ? `${matched.Car},${matched.Jeep},${matched.Truck},${matched.Tricycle},${matched.Motorcycle},${matched.Bus}`
          : '0,0,0,0,0,0';
        return `${p.timestamp.toISOString()},${p.pm25},${tier},${vRow}`;
      })
      .join('\n');
